...
class DiceRoll(IconScoreBase):
    ...
    def set_treasury(self) -> None:
        Logger.info(f'{self.msg.value} was added to the treasury from address {self.msg.sender}', TAG)

    @external(readonly=True)
    def get_treasury(self) -> int:
        Logger.info(f'Amount in the treasury is {self.icx.get_balance(self.address)}', TAG)
    ...
    def fallback(self):
        ...
        if (self.icx.get_balance(self.address)) < 2 * amount:
            Logger.info(f'Not enough in treasury to make the play.', TAG)
            revert('Not enough in treasury to make the play.')

        # do the flip.
        win = int.from_bytes(sha3_256(self.msg.sender.to_bytes() + str(self.block.timestamp).encode()), "big") % 6
        Logger.info(f'Result of dice roll was {win}.', TAG)
        ...
        if win == 1:
            ...
            try:
                ...
                Logger.info(f'Sent winner ({self.msg.sender}) {payout}.', TAG)
            except:
                Logger.info(f'Send failed.', TAG)
